...
class vmmConnection(object):
    ...
    def _init_virtconn(self):
        ...
        def clear_cache(pools=False):
            ...
            self._init_objects(pollpool=True)
    ...
    def host_memory_size(self):
        if not self._backend.is_open() or not self._hostinfo:
            return 0
    ...
    def _domain_lifecycle_event(self, conn, domain, event, reason, userdata):
        ...
        if obj:
            ...
            self._init_objects(pollvm=True, force=True)
    ...
    def _network_lifecycle_event(self, conn, network, event, reason, userdata):
        ...
        if obj:
            ...
            self._init_objects(pollnet=True, force=True)
    ...
    def _storage_pool_lifecycle_event(self, conn, pool,
        ...
        if obj:
            if is_event_refreshed:
                obj.refresh_pool_cache_from_event_loop()
            else:
                obj.recache_from_event_loop()
        else:
            self._init_objects(pollpool=True, force=True)
    ...
    def _add_conn_events(self):
        ...
        # TODO:storage poll events
        logging.debug("#NOT# Using storage pool events")
        # try:
        #     if FORCE_DISABLE_EVENTS:
        #         raise RuntimeError("FORCE_DISABLE_EVENTS = True")
    ...
    def _open_loop(self):
        try:
            ...
            self._change_state(is_active and self._STATE_ACTIVE or self._STATE_DISCONNECTED)
        except Exception, e:
            is_active = False
            self.close()
    ...
    def _populate_initial_state(self):
        ...
        self._populate_objects(stats_update=True,
    ...
    def _new_object_cb(self, obj, initialize_failed):
        ...
        try:
            ...
            logging.debug("Faild to add object" + str(e))
            pass
    ...
    def _populate_objects(self, *args, **kwargs):
        e = None
        try:
            self._init_objects(*args, **kwargs)
    ...
    def _init_objects(self, stats_update=False, pollvm=False, pollnet=False, pollpool=False, polliface=False, pollnodedev=False, force=False, initial_poll=False):
        if self._closing:
            return
        if self.is_disconnected():
            return
        ...
        self._gone_object_signals(gone_objects)
        ...
        for obj in preexisting_objects:
            try:
                ...
                if (isinstance(e, libvirt.libvirtError) and
                    (getattr(e, "get_error_code")() == libvirt.VIR_ERR_SYSTEM_ERROR)):
    ...
    def _poll(self, initial_poll, pollvm, pollnet, pollpool, polliface, pollnodedev):
        gone_objects = []
        preexisting_objects = []
        ...
        for newlist in [new_vms, new_nets, new_pools, new_ifaces, new_nodedevs]:
            if not newlist:
                continue
            for obj in newlist:
                initialize_failed = obj.init_libvirt_state()
                self._new_object_cb(obj, initialize_failed)
