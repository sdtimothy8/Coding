...
class Plugin(UnicornPlugin):
    ...
    def __init__(self, args=None):
        """Init the plugin."""
        UnicornPlugin.__init__(self, args=args, items_history_list=items_history_list)
    ...
    def update(self):
        ...
        if self.input_method == 'local':
            ...
            # available: the actual amount of available memory that can be given instantly to processes that request more
            # memory in bytes; this is calculated by summing different memory values depending on the platform
            ...
            # the actual memory available (use ‘available’ instead).
