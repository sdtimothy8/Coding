...
class SessionAuthticationMiddleware(object):
    ...
    def process_request(self, request):
        ...
        token = request.META.get(settings.REQUEST_TOKEN_NAME, None)
        current_time = time.time()

        # delete expire auth
        deleteExpireAuth(token, current_time)

        # fetch new auth
        permission = KSMP_Permission()
        pms = permission.get_permission(token)
        authlist = permission.get_all_auth()
        data = {}
        urls = ['/login/validatecode/', '/monitor/cpu/load/', '/monitor/mem/', '/resources/psnetinfo/', '/monitor/disk/io/',
                '/monitor/cpu/load/', '/monitor/cpu/baseinfo/', '/monitor/swap/', '/monitor/fs/basic/',
                '/monitor/cpu/sensor/', '/resources/pslist/']
        if pms in authlist:
            ...
            try:
                ...
                if isvalidpassword(username, password):
                    ...
                    request.META[settings.TOKEN_NAME] = authcode
                    userStatus.initUserStatus(username)
    ...
    def process_response(self, request, response):
        ...
        token = request.META.get(settings.REQUEST_TOKEN_NAME, None)
        if token is None:
            token = request.META.get(settings.TOKEN_NAME, None)
        if token is not None:
            response[settings.TOKEN_NAME] = token
...
def deleteExpireAuth(token, current_time):
    '''
