...
private:
	...
	string* first_free;
	string* cap;
	// use static cause compile error
...
{
	//1) resize n < size, 析构掉部分元素; 更新数据结构
	//2) resize size < n < capacity, 初始化部分元素，更新数据结构
	//3) n > capacity; allocate n's memory; copy from old to new; release old mem;
...
{
	// 1) n < capacity, do nothing
	// 2) n > capacity, allocate new mem of n, 
	// copy the elements to new addr
	if (n > capacity())
	{
		auto newdata = alloc.allocate(n);
		auto dest = newdata;
...
void printStrVecInfo(const StrVec& strVec )
{
	for(auto iter : strVec)
		std::cout << iter << std::endl;
	std::cout << "size() is: " << strVec.size() << std::endl;
	std::cout << "capacity() is: " << strVec.capacity() << std::endl;
}
